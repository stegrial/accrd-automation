...
#   И Оставляет пустое поле поиска
    ...
    И Вводит "Qq-Йй. #_!"
    Тогда Видит что кнопка "Найти" недоступна
    Тогда Видит сообщение "ФИО может содержать только русские буквы и пробелы"
  ...
  # Исчерпан лимит на количество сделок для клиента, заявка не может быть подтверждена
  @test
  Сценарий: Поиск неподтвержденной и подтвержденной заявок по ФИО
    Допустим "Пользователь 3 клиент 2" открывает страницу с формой поиска клиента по номеру счета
    ...
    И Заполняет форму используя способ покупки - У застройщика и "набор данных 5"
    ...
    Допустим "Пользователь 3 клиент 2" открывает страницу с формой поиска заявки по ФИО
    И Вводит "Литвинов Иван Петрович"
    И Нажимает на кнопку "Найти"
    Тогда Видит что созданная заявка отсутствует в списке заявок
    Допустим "Пользователь 3 клиент 2" открывает страницу с формой поиска заявки по номеру
  ...
  Сценарий: Прикрепление документов если заявка не подтверждена
    ...
    Тогда Видит что заявка не доступна для приложения документов
